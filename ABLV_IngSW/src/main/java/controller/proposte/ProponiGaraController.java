...
public class ProponiGaraController {
    ...
    @FXML private Spinner<Integer> numProvaSpinner = new Spinner<>();
    @FXML private Spinner<Integer> minPersoneSpinner = new Spinner<>();
    @FXML private Spinner<Integer> maxPersoneSpinner = new Spinner<>();
    @FXML private ComboBox<Tecnica> tecnicaBox = new ComboBox<>();
    @FXML private ComboBox<TipologiaGara> tipologiaBox = new ComboBox<>();
    @FXML private ComboBox<Campionato> campionatoBox = new ComboBox<>();
    @FXML private ComboBox<Arbitro> arbitroBox = new ComboBox<>();
    @FXML private ComboBox<CampoGara> campoGaraBox = new ComboBox<>();
    @FXML private ComboBox<CriterioPunti> criterioBox = new ComboBox<>();
    @FXML private DatePicker dataPicker = new DatePicker();
    @FXML private Button proponiButton;
    
    private boolean isAdjusting = false;
    private ProponiGaraService proponiService = new ProponiGaraService();
    ...
    private void caricaDatiDatabase() {
    	try {
            ...
            campionatiList = FXCollections.observableArrayList(proponiService.caricaCampionati());
            campionatoBox.setItems(campionatiList);
            
            // Carica gli arbitri dal database
            arbitriList = FXCollections.observableArrayList(proponiService.caricaArbitri());
            arbitroBox.setItems(arbitriList);
            
            // Carica i campi gara dal database
            campiGaraList = FXCollections.observableArrayList(proponiService.caricaCampiGara());
