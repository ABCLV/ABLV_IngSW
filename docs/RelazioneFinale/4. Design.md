...
#### 3.2 Architetture MVCS
#### 3.3 Viste architetturali
#### 3.4 Componenti e connettori
...
Il sistema è stato progettato secondo un’architettura MVCS, adottata per separare chiaramente le responsabilità, facilitare il testing e rendere il software più manutenibile.

Questa struttura è riflessa direttamente nell’organizzazione dei package e nei diagrammi UML presentati.

**Model**: rappresenta il dominio applicativo che contiene
* Entità principali (come Gara, Campionato, ...)
* Regole di business (calcolo dei punteggi e gestione delle classifiche)
* Stato e relazioni tra le entità

**View**: gestisce il modo in cui vengono mostrati i dati all'utente e come il sistema interagisce con esso
* Non contiene la logica del sistema
* Raccoglie gli input dell'utente
* Visualizza le informazioni del Model

**Controller**: intermediario tra View e Service
* Riceve gli input dalle View
* Valida le richieste
* Invoca i flussi appropriati per ogni operazione
* Coordina il flusso applicativo

**Service**: incapsula la logica applicativa e coordina l'esecuzione dei casi d'uso
* Implementa i casi d'uso del sistema
* Coordina le modifiche al Model
* Rappresenta il punto di accesso principale alla logica del sistema per i Controller
